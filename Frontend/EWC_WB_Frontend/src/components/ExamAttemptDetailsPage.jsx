...
/**
 * Word-level diff — returns an array of { word, status } where
 * status is 'same' | 'removed' | 'added'.
 *
 * We use a simple longest-common-subsequence approach on word tokens
 * (case-insensitive, punctuation-stripped for matching).
 */
function wordDiff(original, corrected) {
  const strip  = (w) => w.replace(/[.,!?;:()"']/g, '').toLowerCase();
  const origTok = tokenise(original);
  const corrTok = tokenise(corrected);

  // LCS on stripped non-whitespace words
  const origWords = origTok.filter((t) => t.trim());
  const corrWords = corrTok.filter((t) => t.trim());
  ...
  // back-track to build match table for original indices
  const matchedOrig = new Set();
  const matchedCorr = new Set();
  let i = m, j = n;
  while (i > 0 && j > 0) {
    if (strip(origWords[i - 1]) === strip(corrWords[j - 1])) {
      matchedOrig.add(i - 1);
  ...
  // Build annotated original tokens
  let wi = 0;
  const origAnnotated = origTok.map((tok) => {
    if (!tok.trim()) return { word: tok, status: 'space' };
    const status = matchedOrig.has(wi) ? 'same' : 'removed';
    wi++;
    return { word: tok, status };
  });

  // Build annotated corrected tokens
  let wj = 0;
  const corrAnnotated = corrTok.map((tok) => {
    if (!tok.trim()) return { word: tok, status: 'space' };
    const status = matchedCorr.has(wj) ? 'same' : 'added';
    wj++;
    return { word: tok, status };
  });

  return { origAnnotated, corrAnnotated };
}

// ─── DiffText components ──────────────────────────────────────────────────────

function OriginalDiffText({ original, corrected }) {
  if (!original) return <span className="text-muted-foreground italic text-sm">No answer provided.</span>;

  const { origAnnotated } = wordDiff(original, corrected);

  return (
    <span className="text-sm leading-relaxed">
      {origAnnotated.map((tok, i) => {
        if (tok.status === 'space') return <span key={i}>{tok.word}</span>;
        if (tok.status === 'removed') {
          return (
            <mark
              key={i}
              title="Error / removed in correction"
              className="bg-red-100 text-red-800 dark:bg-red-900/35 dark:text-red-300
                         rounded-sm px-0.5 line-through decoration-red-500 decoration-2
                         underline underline-offset-2 cursor-help"
            >
              {tok.word}
            </mark>
          );
...
function CorrectedDiffText({ corrected, original }) {
  if (!corrected) return <span className="text-muted-foreground italic text-sm">No corrected text available.</span>;

  const { corrAnnotated } = wordDiff(original, corrected);

  return (
    <span className="text-sm leading-relaxed">
      {corrAnnotated.map((tok, i) => {
        if (tok.status === 'space') return <span key={i}>{tok.word}</span>;
        if (tok.status === 'added') {
          return (
            <mark
              key={i}
              title="Correction / new word"
              className="bg-emerald-100 text-emerald-800 dark:bg-emerald-900/35 dark:text-emerald-300
                         rounded-sm px-0.5 font-medium cursor-help"
            >
              {tok.word}
            </mark>
          );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        ...
        <p className={`text-xs font-bold ${scoreTone(numeric)}`}>{numeric}%</p>
      </div>
        ...
        <div className="px-5 pb-5 space-y-4 border-t border-border/40">
          ...
          {hasDiff ? (
            <div className="rounded-2xl glass-sm overflow-hidden">
              {/* Legend */}
              <div className="flex items-center gap-4 px-4 py-2.5 glass-sm border-b border-white/20 dark:border-white/8 text-xs text-muted-foreground" style={{borderRadius:'0.5rem 0.5rem 0 0'}}>
                <span className="font-semibold text-foreground">Text Comparison</span>
                <span className="flex items-center gap-1.5">
                  <span className="inline-block w-3 h-3 rounded-sm bg-red-200 dark:bg-red-900/50 border border-red-300 dark:border-red-700" />
                  Error (removed)
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="inline-block w-3 h-3 rounded-sm bg-emerald-200 dark:bg-emerald-900/50 border border-emerald-300 dark:border-emerald-700" />
                  Fixed (added)
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 divide-y xl:divide-y-0 xl:divide-x divide-border/40">
                ...
                <div className="p-4 bg-red-50/30 dark:bg-red-950/10">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-semibold text-red-600 dark:text-red-400 uppercase tracking-wider">Your Answer</span>
                  </div>
                  <div className="text-foreground leading-relaxed whitespace-pre-wrap">
                    <OriginalDiffText original={origText} corrected={corrText} />
                  </div>
                </div>
                ...
                <div className="p-4 bg-emerald-50/30 dark:bg-emerald-950/10">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Corrected Version</span>
                  </div>
                  <div className="text-foreground leading-relaxed whitespace-pre-wrap">
                    <CorrectedDiffText corrected={corrText} original={origText} />
                  </div>
                </div>
              </div>
            </div>
          ) : (
            /* Fallback: no diff, show plain text */
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
              <div className="rounded-xl glass-sm p-4">
                <p className="text-xs text-muted-foreground mb-2 flex items-center gap-2">
                  <Languages className="w-3.5 h-3.5" /> Your Answer
                </p>
                <p className="text-sm text-foreground leading-relaxed whitespace-pre-wrap">
                  {origText || <span className="text-muted-foreground italic">No answer text</span>}
                </p>
              </div>
              <div className="rounded-xl glass-sm p-4">
                <p className="text-xs text-muted-foreground mb-2 flex items-center gap-2">
                  <Sparkles className="w-3.5 h-3.5" /> Corrected Text
                </p>
                <p className="text-sm text-foreground leading-relaxed whitespace-pre-wrap">
                  {corrText || <span className="text-muted-foreground italic">No corrected text</span>}
                </p>
              </div>
            </div>
          )}
