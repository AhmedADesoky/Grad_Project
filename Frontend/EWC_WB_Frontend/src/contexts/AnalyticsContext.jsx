...
import { getUserFeedback, getRecentClassifications, getUserExamAttempts, getUserActivityDates } from '../graphql/AIService';
...
export function AnalyticsProvider({ children }) {
  const { user } = useAuth();
  const { activeMode } = usePlan();

  // Use the same multi-fallback ID resolution as PlanContext/PersonalizedPlan
  ...
  const refreshAnalytics = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [fbRes, clsRes, attemptsRes, activityRes] = await Promise.allSettled([
        getUserFeedback({ User_Id: userId, Limit: 50, Offset: 0 }),
        getRecentClassifications({ User_Id: userId, Days: 30 }),
        getUserExamAttempts({ User_Id: userId, Limit: 50 }),
        // Pass the active plan mode so dates are tied to the user's chosen plan type
        getUserActivityDates({ User_Id: userId, Days: 30, Mode: activeMode || undefined }),
      ]);
      ...
      setActivityDates(activityRes.status === 'fulfilled' ? (activityRes.value || []) : []);
    } finally {
      setLoading(false);
    }
  }, [userId, activeMode]);
  ...
  useEffect(() => {
    if (userId) refreshAnalytics();
  }, [userId, activeMode]);

  const VALID_STATUSES = new Set(['SUBMITTED', 'AUTO_SUBMITTED']);

  const normalizedAttempts = useMemo(() => {
    return [...examAttempts]
      .filter((a) => VALID_STATUSES.has(a?.Status))   // drop IN_PROGRESS / CANCELLED
      .map((a) => ({
        ...a,
