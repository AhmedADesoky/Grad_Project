...
function TaskRow({ task }) {
  ...
  return (
    <Link to="/plan"
      className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl glass-sm hover:shadow-md transition-spring ${cancelled ? 'opacity-50' : ''}`}>
      <div className={`flex-shrink-0 ${done ? 'text-emerald-500' : cancelled ? 'text-red-400/60' : 'text-muted-foreground/30'}`}>
        {done ? <CheckCircle className="w-[18px] h-[18px]" /> : <Circle className="w-[18px] h-[18px]" />}
      </div>
      <div className="min-w-0 flex-1">
        <p className={`text-[13px] font-medium leading-tight truncate ${done || cancelled ? 'text-muted-foreground line-through' : 'text-foreground'}`}>{task.title}</p>
      ...
      <span className={`flex-shrink-0 text-[10px] font-semibold px-2.5 py-1 rounded-full ${
        ...
        : cancelled ? 'bg-red-100 dark:bg-red-900/20 text-red-500/70 dark:text-red-400/70'
        : `${type.bg} ${type.text}`
      }`}>{done ? 'Done' : cancelled ? 'Skipped' : type.label}</span>
    </Link>
  );
}
...
export function MainPage() {
  const { user }    = useAuth();
  const { metrics, examAttempts = [], feedback = [], refreshAnalytics } = useAnalytics();
  const { planData, loading: loadingPlan, refreshPlan } = usePlan();
  ...
  useEffect(() => {
    refreshPlan();      // only re-fetches if data is stale (> 5 min old)
    refreshAnalytics();
  }, [location.key]);
  ...
  const remaining   = todayTasks.filter(t => t.status !== 'submitted' && t.status !== 'reviewed' && t.status !== 'cancelled').length;
            ...
            {loadingPlan ? (
              ...
              <div className="space-y-2">
                ...
                <div className="pt-2 flex items-center justify-between text-[11px] text-muted-foreground/60">
                  <span>{doneToday} of {todayTasks.filter(t => t.status !== 'cancelled').length} completed</span>
                  {remaining > 0
