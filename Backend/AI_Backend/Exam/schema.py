...
def _deduplicate_text(text: str) -> str:
    """Remove duplicate paragraphs before scoring to avoid inflated AI probability."""
    import re
    ...
    for p in paragraphs:
        ...
        words = p_norm.split()
        if not any(
            len(set(words) & set(prev.split())) / max(len(words), 1) > 0.8
...
def _check_ai(text):
    """Run AI detection on deduplicated text.
    Returns (is_ai: bool, ai_probability: float).
...
def compute_relevance_score(question_text, answer_text):
    ...
    Replaces the old keyword-overlap approach which penalised paraphrasing:
    a well-written answer that avoids repeating the question's exact words
    now scores as well as a literal one.
    """
    ...
    try:
        ...
        tfidf = vect.fit_transform([question_text, answer_text])
        similarity = float(cos_sim(tfidf[0:1], tfidf[1:2])[0][0])
        # Scale: raw cosine for short text pairs rarely exceeds 0.5 even for
        # highly relevant answers, so rescale to the [0, 100] range with a
        # floor of 40 to avoid unfairly penalising legitimate paraphrasing.
        score = 40.0 + 60.0 * clamp(similarity / 0.5, 0.0, 1.0)
        return round(score, 2)
    except Exception:
        # Graceful fallback to neutral score if sklearn is unavailable.
        return 70.0
...
def compute_language_score(feedback_data):
    """Overall score from the feedback model, used directly.

    The old formula added 0.1×grammar + 0.1×vocab + 0.1×punct on top of
    0.7×overall, but overall is already a weighted combination of those three
    dimensions — so they were counted twice.  Using overall alone is simpler
    and avoids the artificial inflation.
    """
    overall = float(feedback_data.get("overall_score", 0.0))
    return round(clamp(overall, 0.0, 100.0), 2)
...
class Generate_Exam_Questions_Mutation(Mutation):
    ...
    @require_auth
    @rate_limit(max_calls=3, window_seconds=86400)  # 3 exam starts per 24 hours
    def mutate(self, info, User_Id, Level=None, Count=4, Duration_Minutes=None):
        try:
            # ── 80% plan-completion gate ──────────────────────────────────────
            # Rule: if the user has NO active plan → allow (no plan = first exam
            # ever, or plan was completed/expired). If the user HAS an active plan
            # → they must complete ≥80% of the current period first.
            import json as _json
            from Personalized_Plan.models import Personalized_Plan

            try:
                # djongo can't translate BooleanField=True into valid SQL — filter
                # only by User_Id and check Is_Active in Python instead.
                _all_plans = list(
                    Personalized_Plan.objects
                    .filter(User_Id=User_Id)
                    .order_by("-Created_At")
                )
                active_plan = next((p for p in _all_plans if p.Is_Active), None)
            except Exception as _pe:
                logger.exception("Exam gate: could not query active plan — allowing user=%s: %s", User_Id, _pe)
                active_plan = None  # DB error → allow (we can't confirm a plan exists)

            if active_plan:
                # Active plan found — enforce the 80% gate.
                # Any exception here → fail CLOSED (we know a plan exists).
                try:
                    raw_plan = active_plan.Plan or {}
                    if isinstance(raw_plan, str):
                        try:
                            raw_plan = _json.loads(raw_plan)
                        except Exception:
                            raw_plan = {}

                    plan_list = raw_plan.get("plan", []) if isinstance(raw_plan, dict) else []
                    current_idx = int(active_plan.Current_Period_Index or 0)
                    current_period = plan_list[current_idx] if current_idx < len(plan_list) else None

                    if not current_period:
                        logger.warning(
                            "Exam gate: user=%s plan has no readable period at index %d — blocking",
                            User_Id, current_idx,
                        )
                        return Generate_Exam_Questions_Mutation(
                            Success=False, Questions=[], Error="GATE_ERROR",
                        )

                    period_name = current_period.get("period", f"Week {current_idx + 1}")

                    # Count tasks from plan JSON — always in sync with submissions
                    done_statuses = {"submitted", "completed", "done", "reviewed"}
                    total = 0
                    done = 0
                    for day in current_period.get("days", []):
                    ...
                    if total == 0:
                        logger.warning(
                            "Exam gate: user=%s period=%s has 0 tasks — blocking",
                            User_Id, period_name,
                        )
                        return Generate_Exam_Questions_Mutation(
                            Success=False, Questions=[], Error="GATE_ERROR",
                    ...
                    logger.info(
                        "Exam gate: user=%s period=%s done=%d/%d (%d%%)",
                        User_Id, period_name, done, total, pct,
                    )
                    if pct < 80:
                        return Generate_Exam_Questions_Mutation(
                            Success=False, Questions=[],
                            Error=f"PLAN_INCOMPLETE:{pct}:{done}:{total}",
                        )

                except Exception as _gate_err:
                    logger.exception(
                        "Exam gate completion check error — blocking user=%s: %s", User_Id, _gate_err
                    )
                    return Generate_Exam_Questions_Mutation(
                        Success=False, Questions=[], Error="GATE_ERROR",
                    )
            else:
                logger.info("Exam gate: user=%s has no active plan — allowing", User_Id)
            # ─────────────────────────────────────────────────────────────────
...
class Evaluate_Exam_Mutation(Mutation):
    ...
    def mutate(self, info, User_Id, Answers, Save_To_Database=True, Attempt_Id=None):
        try:
            ...
            pass_threshold = clamp(float(os.getenv("EXAM_PASS_PERCENTAGE", "60")), 0.0, 100.0)
            ...
            # ── AI detection pre-scan (single pass over all answers) ────────────
            # Concatenate all non-empty answers into one text block and run a
            # single model inference instead of one per answer. This is faster
            # (one forward pass vs. four) and fairer (overall writing style is
            # judged rather than any single short answer tripping the threshold).
            all_answer_text = "\n\n".join(
            ...
            if all_answer_text:
                try:
                    is_ai, ai_prob = _check_ai(all_answer_text)
                except Exception as det_err:
                    logger.error("AI detector crashed during exam scan — rejecting: %s", det_err)
                    return Evaluate_Exam_Mutation(
                        Success=False,
                        AI_Detected=False,
                        AI_Confidence=0.0,
                ...
                if is_ai:
                    ...
                    return Evaluate_Exam_Mutation(
                        ...
                        Error=(
                            ...
                            f"({ai_prob:.1f}% confidence). Your exam has been cancelled."
                        ),
                    )
            ...
            for a in Answers:
                ...
                item = {
                    ...
                    "Feedback_Spelling_Score":  float(feedback_data.get("spelling_score",  0.0)),
            ...
            if Save_To_Database and attempt:
                ...
                attempt.AI_Detected = False
                attempt.AI_Confidence = 0.0
                attempt.save()
