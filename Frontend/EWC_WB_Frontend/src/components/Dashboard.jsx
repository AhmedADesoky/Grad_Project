...
function getStatus(attempt) {
  ...
  if (attempt?.Passed || s >= 70) return { text: 'Passed',      cls: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300' };
...
function ActivityHeatmap({ dates }) {
  ...
  const counts = useMemo(() => {
    const map = {};
    dates.forEach(v => { const k = dateKey(v); if (k) map[k] = (map[k] || 0) + 1; });
    return map;
  }, [dates]);
  ...
  const cells = useMemo(() => {
    ...
    for (let i = totalDays - 1; i >= 0; i--) {
      ...
      list.push({ key: k, count: counts[k] || 0, date: d });
    }
    return list;
  }, [counts]);
  ...
  const months = [];
  weeks.forEach((w, wi) => {
    const m = w[0].date.toLocaleDateString(undefined, { month: 'short' });
    if (wi === 0 || m !== months[months.length - 1]?.label) months.push({ label: m, col: wi });
  });
  ...
  return (
    <div>
      <div className="flex gap-0.5 mb-1">
        {months.map(m => (
          <div key={m.col + m.label}
            className="text-[9px] text-muted-foreground/50 font-medium"
            style={{ gridColumn: m.col + 1, minWidth: 0, flex: 1 }}>
            {m.label}
          </div>
        ))}
      </div>
      <div className="flex gap-[3px]">
        {weeks.map((week, wi) => (
          <div key={wi} className="flex flex-col gap-[3px]">
            {week.map(cell => (
              <div key={cell.key} title={`${cell.key}: ${cell.count} session${cell.count !== 1 ? 's' : ''}`}
                className={`w-[9px] h-[9px] rounded-[2px] transition-colors ${intensity(cell.count)}`} />
            ))}
          </div>
        ))}
      </div>
      <div className="flex items-center gap-1.5 mt-2">
        ...
        {['bg-muted/40','bg-primary/25','bg-primary/50','bg-primary/80'].map((c,i) => (
...
export function Dashboard() {
  ...
  const { user } = useAuth();
  const { metrics, feedback = [], classifications = [], examAttempts = [], loading } = useAnalytics();
  ...
  const completionPct      = useMemo(() => totalAttempts ? Math.round(attemptsSorted.filter(i => i?.Passed || toNumber(i?.Percentage) >= 70).length / totalAttempts * 100) : 0, [attemptsSorted]);
  ...
  // activity heatmap dates
  const activityDates = useMemo(() => [
    ...attemptsSorted.map(a => a?.Submitted_At || a?.Created_At),
    ...feedback.map(f => f?.Created_At),
  ].filter(Boolean), [attemptsSorted, feedback]);
    ...
    <PageLayout>
      <div className="space-y-4">
          ...
          <div className={G + ' lg:col-span-2 p-5'}>
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3 text-[10px] text-muted-foreground/60">
                ...
                <span className="flex items-center gap-1"><span className="inline-block w-5 border-t border-dashed border-muted-foreground/40"/>&nbsp;Pass (70%)</span>
              </div>
            </div>
            {progressData.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={progressData} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
                  ...
                  <ReferenceLine y={70} stroke="var(--muted-foreground)" strokeDasharray="5 4" strokeOpacity={0.45} />
          ...
          <div className={G + ' p-5'}>
            ...
            {skillTrendData.length > 0 ? (
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={skillTrendData} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" opacity={0.25} />
                  <XAxis dataKey="label" stroke="var(--muted-foreground)" tick={{ fontSize: 11 }} tickLine={false} axisLine={false} />
                  <YAxis domain={[0, 100]} stroke="var(--muted-foreground)" tick={{ fontSize: 11 }} tickLine={false} axisLine={false} />
                  <Tooltip contentStyle={TT} cursor={{ stroke: 'var(--border)', strokeWidth: 1 }} />
                  <Line type="monotone" dataKey="Vocabulary"  stroke="#f59e0b" strokeWidth={2}   dot={{ r: 3, fill: '#f59e0b' }} activeDot={{ r: 5 }} />
                  <Line type="monotone" dataKey="Punctuation" stroke="#3b82f6" strokeWidth={2}   dot={{ r: 3, fill: '#3b82f6' }} activeDot={{ r: 5 }} />
                  <Line type="monotone" dataKey="Grammar"     stroke="#10b981" strokeWidth={2.5} strokeDasharray="6 3" dot={{ r: 4, fill: '#10b981', stroke: '#fff', strokeWidth: 1.5 }} activeDot={{ r: 6 }} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
            ...
            {totalAttempts > 0 ? (
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={histData} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
                  ...
                  <ReferenceLine x="70–79" stroke="var(--primary)" strokeDasharray="4 3" strokeOpacity={0.5} />
        ...
        <section className={G + ' p-5'}>
          ...
          <h3 className="text-[15px] font-bold text-foreground mb-4">Study activity — last 15 weeks</h3>
                        ...
                        <td className="py-3.5 px-3">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div className="w-14 h-[4px] rounded-full bg-muted/50 overflow-hidden">
                              <div className={`h-full rounded-full ${row.score >= 70 ? 'bg-emerald-500' : 'bg-amber-500'}`} style={{ width: `${row.score}%` }} />
                            </div>
                          </div>
                        </td>
                        <td className="py-3.5 px-3">
