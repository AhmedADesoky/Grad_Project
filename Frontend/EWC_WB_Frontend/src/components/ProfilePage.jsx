...
export function ProfilePage() {
  ...
  // Fetch real plan progress
  const [planProgress, setPlanProgress] = useState(null);
  useEffect(() => {
    if (!userId) return;
    (async () => {
      try {
        const planData = await getActivePlan({ User_Id: userId, Mode: 'weekly' });
        if (!planData) return;
        ...
        setPlanProgress(allTs.length > 0 ? Math.round((done / allTs.length) * 100) : 0);
      } catch { /* ignore */ }
    })();
  }, [userId]);
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="relative flex-shrink-0 group" style={{ width:80, height:80 }}>
              ...
              <div className="absolute rounded-full overflow-hidden bg-primary text-primary-foreground flex items-center justify-center text-[18px] font-bold" style={{ inset:6 }}>
                {user?.profileImage ? <img src={user.profileImage} alt={displayName} className="w-full h-full object-cover"/> : initials}
              </div>
              <button
        ...
        {showRemove && (
          <div className={glass + ' p-4'}>
            ...
            <div className="flex gap-2">
              <button onClick={() => { updateProfile({ profileImage: undefined }); setShowRemove(false); }}
