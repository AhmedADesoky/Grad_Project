...
import { sendOTPEmail, sendWelcomeEmail, sendPasswordResetEmail } from './emailService.js';
...
export const updateProfile = async ({
  ...
  if (Profile_Image !== undefined) {
    if (!Profile_Image) {
      // Remove image — delete from Cloudinary if it was one we uploaded
      if (user.Profile_Image && user.Profile_Image.includes('cloudinary')) {
        const publicId = user.Profile_Image.split('/').pop().split('.')[0];
        await cloudinary.uploader.destroy(`ewc_profiles/${publicId}`).catch(() => {});
      }
      user.Profile_Image = null;
    } else if (Profile_Image.startsWith('data:image/')) {
      // Upload base64 to Cloudinary and store the secure URL instead
      ...
      // Already a URL (e.g. Google OAuth photo) — store as-is
...
export const requestPasswordReset = async (email) => {
  ...
  if (!user || !user.Is_Verified) return true;

  // Google-only accounts have no password to reset
  if (user.Auth_Provider === 'google' && !user.Password) return true;

  const rawToken = crypto.randomBytes(32).toString('hex');
  const tokenHash = hashToken(rawToken);

  user.Password_Reset_Token = tokenHash;
  user.Password_Reset_Expires = new Date(Date.now() + RESET_TOKEN_EXPIRY_MS);
  await user.save();
...
export const resetPassword = async (rawToken, newPassword) => {
  if (!rawToken || !newPassword) throw new Error('Token and new password are required');
...
export const queueExamSubmission = async ({ User_Id, Attempt_Id, Answers }) => {
  if (!Attempt_Id) throw new Error('Attempt_Id is required to submit an exam');
  const user = await User.findById(User_Id);
  if (!user) throw new Error('User not found');

  // Queue immediately — AI detection and evaluation happen inside the worker
  ...
  return { queued: true, jobId: job.id, ai_detected: false, ai_confidence: 0 };
};
...
export const addExamScore = async ({ User_Id, Score, Level }) => {
  ...
  const levelValue = typeof Level === 'string' && Level.trim().length > 0
    ? Level.trim()
    : null;
  ...
  // Only update the user's Level if the exam returned a valid classification.
  // Never overwrite with null or a fallback — that would silently downgrade.
  if (levelValue) {
